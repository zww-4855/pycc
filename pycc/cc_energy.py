...
def get_CC_FOURTHO_parQ(T2,T3,o,v,D4,D2,W):
    ...
    print('full (Q) ccsd T3^W D4 WT2^2:',energy,energy/24,energy/36)
    print(flush=True)
    print('non zero elements:', np.count_nonzero(t4_wnT2sqr),np.count_nonzero(d4t4_wnT3))
    print(flush=True)

    newE= build_sqrbrak_corrections.build_SIXTHO_sqrBrakQuads(d4t4_wnT3*D4,d4t4_wnT3.transpose(4,5,6,7,0,1,2,3),o,v)
    print('final T3^W D4 WT3:',newE,newE/24,newE/36)
############# intermediate step
    ...
    print(flush=True)
    T4dag = d4t4_wnT3.transpose(4,5,6,7,0,1,2,3)
    r = -0.083333333 * np.einsum("ijkabc,abdeijkl,lcde->",T3,T4dag,W[o,v,v,v],optimize="optimal")
    r += -0.083333333 * np.einsum("ijkabc,abcdijlm,lmkd->",T3,T4dag,W[o,o,o,v],optimize="optimal")
    print('Another way to computer T3^W D4 WT3:',r)
...
#    print('WnT2^3 contribution to (Qf) faster T2 route:',teste)
    ...
    # Test wicked _toT4 part:
#    D4T4 = qf.wnT2cubed_toT4(W,T2,o,v)
#    D4T4 = D4T4.transpose(4,5,6,7,0,1,2,3)
#    resid_aaaa = (1.0/16.0)*np.einsum('klcd,abcdijkl->abij',T2,D4T4)
#    resid_aaaa = tamps.antisym_T2(resid_aaaa,None,None)
...
def sixthOrderQf_wnT2T3(W,T2,T3,o,v,D2):
    # pdagq version -- factor of 1/2 handled within pdaggerq code, so unnecessary here
    D2T2 = qf.wnT2T3_pdagq(W,T2,T2.transpose(2,3,0,1),T3.transpose(3,4,5,0,1,2),o,v)
    teste=0.25*np.einsum('jiab,abji',W[o,o,v,v]*D2,D2T2)
    print('pdagq WnT2T3 6th order energy:',teste)
...
#    resid_aaaa = (1.0/16.0)*np.einsum('klcd,abcdijkl->abij',T2,D4T4)
#    resid_aaaa = tamps.antisym_T2(resid_aaaa,None,None)
#    sixthO_wnT2T3=0.5*(1.0/4.0)*np.einsum("ijab,abij",W[o,o,v,v]*D2,resid_aaaa)
